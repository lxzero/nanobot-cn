...
所有工具的默认工作目录均为此路径。**优先使用相对路径**（如 `HEARTBEAT.md`、`memory/MEMORY.md`），无需拼写绝对路径。

目录结构：
- `memory/MEMORY.md` — 长期记忆，在此写入需要记住的重要事实
- `memory/HISTORY.md` — 历史日志（可用 grep 搜索），每条记录以 `[YYYY-MM-DD HH:MM]` 开头
