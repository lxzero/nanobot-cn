...
class MattermostConfig(Base):
    ...
    mention_policy: Literal["always", "never"] = "always"
...
class MattermostChannel(BaseChannel):
    ...
    async def _handle_posted(self, event_data: dict) -> None:
        ...
        should = self._should_respond(channel_type, channel_id, message, mentions)
        logger.debug(
            "Mattermost respond check: channel_type={}, mention_policy={}, mentions={}, bot_id={}, should={}",
            channel_type, self.config.mention_policy, mentions, self._bot_user_id, should,
        )
        if not is_bot_command and not should:
            return
